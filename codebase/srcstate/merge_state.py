...
densityDF = densityDF.drop(["geo_id"], axis=1)
...
print(cols)
cols = ['State', 'state_name', 'Cases', 'Deaths', 'pop_stay_at_home', 'pop_not_stay_at_home', 'trips', 'TempMedian', 'TempMean', 'PrcpMedian', 'PrcpMean', 'population', 'pop_density']
finalMerge = finalMerge[cols]
finalMerge.columns = finalMerge.columns.str.strip().str.lower()
print(finalMerge.columns)
finalMerge.to_csv(timestr, sep='\t', encoding='utf-8')
