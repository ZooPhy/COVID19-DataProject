...
df = pd.read_csv('climateDataDownload.csv', names = collist, dtype={'FIPS': object}, sep=',')
FIPSMetadata = pd.read_csv('./../covid_state/FIPSMetadata.csv', dtype={'FIPS': object}, sep=',')
df = df.merge(FIPSMetadata)
print(df)
temp_median = df.groupby('State', as_index=False)['Temp'].median()
temp_mean = df.groupby('State', as_index=False)['Temp'].mean()
prcp_median = df.groupby('State', as_index=False)['Prcp'].median()
prcp_mean = df.groupby('State', as_index=False)['Prcp'].mean()
