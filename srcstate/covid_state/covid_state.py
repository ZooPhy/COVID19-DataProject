...
print(mergedData)
mergedData = mergedData.reindex(columns = ['FIPS_x', 'Admin2', 'UID', 'iso2', 'iso3', 'code3', 'State', 'Province_State', 'Country_Region', 'Lat', 'Long_', 'Cases', 'Deaths'])
casesColumn = mergedData.groupby('State', as_index=False)['Cases'].sum()
deathColumn = mergedData.groupby('State', as_index=False)['Deaths'].sum()
stateData = casesColumn.merge(deathColumn, on='State')
stateData.to_csv('covidData.csv', encoding='utf-8')
